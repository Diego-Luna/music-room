...
Légende : **OK** = respectée entièrement · **PARTIEL** · **KO** · *à vérifier*
...
- [x] **Rapide — `handleTrackMoved`** : update `position` **puis sort** lexicographique (indexation fractionnaire). La page détail resync depuis le provider sauf pendant un drag local → le reorder d’un autre user bouge sans refresh.
- [x] **Rapide — `handleTrackAdded`** : filtré par `Track.roomId` (payload Prisma). Sans `roomId`, no-op — plus d’ajout sur toutes les playlists en cache.
